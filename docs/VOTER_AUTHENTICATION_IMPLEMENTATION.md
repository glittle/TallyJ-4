...
2. **SMS Pumping Prevention**
   ...
   - `OnlineVoter` also stores global phone SMS eligibility (`SmsStatus`: null / `"OK"` / block reason). For `VoterIdType` `"P"` + paid channel, a non-null status other than `"OK"` skips Twilio and GreenAPI. The lookup is the phone row (`VoterId` + `VoterIdType == "P"`). WhatsApp presence is a separate `WhatsAppStatus` on that same P row (null / `"OK"` / `no-wa` / `check-failed`). When delivery is WhatsApp, a non-null status other than `"OK"` skips GreenAPI. SMS/voice still send if only `WhatsAppStatus` is blocked.
   - Twilio status callbacks (`POST /api/Public/smsStatus`, v3 `Public/SmsStatus`) auto-learn a lasting block on that P row: terminal `undelivered` / `failed` plus a selected error code (`30003`, `30004`, `30005`, `30006`, `21211`, `21614`) writes `twilio-{code}` when current `SmsStatus` is null or `"OK"`. Missing or unlisted codes do not write a block. SMS `delivered` / voice `completed` on an existing SmsLog SID set that P row to `"OK"` (overwrites a prior block; the phone worked). `sent` is not success. No OnlineVoter or SmsLog is created from a callback. The request must present a valid `X-Twilio-Signature` for `Twilio:AuthToken` (403 otherwise). A successful paid SMS / voice / WhatsApp send writes `SmsLog` (SID, phone, times, initial status) and Twilio sets `StatusCallback` to that endpoint; callbacks update by SID and never insert. Person detail shows recent `SmsLog` rows for that phone. A teller can set `SmsStatus` to `"OK"` or a short block reason on that P row from person detail (same `[Authorize]` as other People writes; SuperAdmin is not a separate people-edit role). See `context/sms-eligibility.md`.
...
4. **Database**
   - `OnlineVoter` - Tracks verification codes and attempts; for phone rows, `SmsStatus` is the global paid-channel eligibility (null = unchecked, `"OK"` = allowed, any other short value = blocked). `WhatsAppStatus` is the separate GreenAPI presence field (null = unchecked, `"OK"` = has WhatsApp, any other short value such as `no-wa` / `check-failed` = do not send WhatsApp). Person create / people import / update with a phone ensures a `VoterIdType` `"P"` row (`VoterId` = the Person phone). Changing the Person phone clears `WhatsAppStatus` on the new number’s P row so it is re-checked. `WhenRegistered` stays null until the first successful code request. Person detail looks up that P row (`VoterId` + `VoterIdType == "P"`) and shows never-seen vs imported-only vs first registered, last login, SMS status, WhatsApp status, and up to five recent `SmsLog` rows for that phone (stored string or +/- variant; not election-scoped). A teller can set that P row’s `SmsStatus` to `"OK"` or a short reason from person detail, and can run GreenAPI `checkWhatsapp` for that stored phone (one person, or check-selected on the People list, max 100, election-scoped). The People list can filter visible rows by that WhatsApp hint (Has WhatsApp / Unchecked / No WhatsApp); send still only queues P-row `"OK"` numbers. A Twilio undelivered/failed callback with a selected lasting-unusable error code stamps `twilio-{code}` on that P row when status is null or `"OK"`. A later SMS `delivered` or voice `completed` callback for an existing SmsLog SID sets that P row to `"OK"`.
