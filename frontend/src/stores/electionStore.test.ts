...
describe("Election Store", () => {
  ...
  describe("computed properties", () => {
    it("should filter active elections correctly", () => {
      const mockElections: ElectionDto[] = [
        { electionGuid: "1", electionStage: "SettingUp" } as ElectionDto,
        { electionGuid: "2", electionStage: "GatheringBallots" } as ElectionDto,
        {
          electionGuid: "3",
      ...
      expect(electionStore.activeElections.map((e) => e.electionGuid)).toEqual([
        "1",
        "2",
      ]);
    });
    ...
    it("should filter finalized elections correctly", () => {
      ...
      expect(
        electionStore.finalizedElections.map((e) => e.electionGuid),
      ).toEqual(["2", "3"]);
    });
  });
  ...
  describe("updateElection", () => {
    it("should update existing election", async () => {
      ...
      expect(electionService.update).toHaveBeenCalledWith("1", {
        name: "Updated Name",
      });
      expect(electionStore.elections[0]).toEqual(updatedElection);
      expect(electionStore.currentElection).toEqual(updatedElection);
  ...
  describe("initializeSignalR", () => {
    ...
    it("should handle SignalR initialization errors gracefully", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Failed to initialize SignalR for election store:",
        expect.any(Error),
      );
  ...
  describe("currentStage", () => {
    ...
    it("returns SettingUp when electionStage is SettingUp", () => {
    ...
    it("returns GatheringBallots when electionStage is GatheringBallots", () => {
      electionStore.currentElection = {
        electionGuid: "1",
    ...
    it("returns ProcessingBallots when electionStage is ProcessingBallots", () => {
      electionStore.currentElection = {
        electionGuid: "1",
        electionStage: "ProcessingBallots",
      } as ElectionDto;
