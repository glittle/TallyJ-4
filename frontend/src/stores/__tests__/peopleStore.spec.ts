  ...
  describe('handlePersonVoteCountUpdated', () => {
    it('should update voteCount for a matching person in the cache', () => {
      ...
      store.candidateCache = [
        ...
        {
          ...createPersonDto({ personGuid: 'person-2', lastName: 'Jones', fullName: 'Bob Jones', voteCount: 0 }),
      ...
      const updated = store.candidateCache.find(p => p.personGuid === 'person-1');
    ...
    it('should not modify other people in the cache', () => {
      ...
      store.candidateCache = [
        ...
        {
          ...createPersonDto({ personGuid: 'person-2', lastName: 'Jones', fullName: 'Bob Jones', voteCount: 3 }),
          _searchText: 'bob jones',
          _soundexCodes: [],
        },
      ] as any;
      ...
      const unchanged = store.candidateCache.find(p => p.personGuid === 'person-2');
    ...
    it('should replace the array reference to invalidate search cache', () => {
      ...
      const initialCache = [
        {
          ...createPersonDto({ personGuid: 'person-1', lastName: 'Smith', fullName: 'Alice Smith', voteCount: 0 }),
          _searchText: 'alice smith',
          _soundexCodes: [],
        },
      ] as any;
    ...
    it('should call getAllForBallotEntry when initializing candidate cache', async () => {
      ...
      const mockPerson = createPersonDto({ personGuid: 'p1', lastName: 'Brown', fullName: 'Ted Brown', canReceiveVotes: false });
      vi.mocked(peopleService.getAllForBallotEntry).mockResolvedValue([mockPerson]);

      const store = usePeopleStore();
      await store.initializeCandidateCache('election-1');

      expect(peopleService.getAllForBallotEntry).toHaveBeenCalledWith('election-1');
    ...
    it('should include ineligible people in candidate cache', async () => {
      const { peopleService } = await import('@/services/peopleService');
      const eligible = createPersonDto({ personGuid: 'p1', lastName: 'Doe', fullName: 'Jane Doe', canReceiveVotes: true });
      const ineligible = createPersonDto({ personGuid: 'p2', lastName: 'Smith', fullName: 'Bob Smith', canReceiveVotes: false, ineligibleReasonCode: 'X01' });
      vi.mocked(peopleService.getAllForBallotEntry).mockResolvedValue([eligible, ineligible]);

      const store = usePeopleStore();
      await store.initializeCandidateCache('election-1');

      expect(store.candidateCache).toHaveLength(2);
      const ineligibleCached = store.candidateCache.find(p => p.personGuid === 'p2');
