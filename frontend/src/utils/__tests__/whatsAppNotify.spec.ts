...
describe("whatsAppNotify", () => {
  it("enables send when some selected people have phones and the list is in bound", () => {
    ...
    expect(
      canNotifySelectedWhatsApp(MAX_WHATSAPP_NOTIFY_SELECTED + 1, 1),
    ).toBe(false);
  });
  ...
  it("maps outcomes to i18n keys", () => {
    ...
    expect(
      whatsAppNotifyOutcomeLabel("skipped-unchecked", (key) => key),
    ).toBe("people.notifyWhatsAppOutcome.skipped-unchecked");
  });
