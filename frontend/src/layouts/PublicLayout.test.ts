...
describe("PublicLayout", () => {
  ...
  it("renders language selector and ThemeSelector components", () => {
    ...
    const hasLanguageSelector = wrapper
      .findComponent({ name: "LanguageSelector" })
      .exists();
    const hasLanguageFlagsSelector = wrapper
      .findComponent({ name: "LanguageFlagsSelector" })
      .exists();
    expect(hasLanguageSelector || hasLanguageFlagsSelector).toBe(true);
    expect(wrapper.findComponent({ name: "ThemeSelector" }).exists()).toBe(
      true,
    );
  });
