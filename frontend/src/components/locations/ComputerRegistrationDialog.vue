...
const rules = reactive<FormRules>({
  ...
  browserInfo: [
    {
      max: 250,
      message: t("locations.computer.browserInfoMaxLength"),
      trigger: "blur",
    },
  ],
  ipAddress: [
    {
      max: 50,
      message: t("locations.computer.ipAddressMaxLength"),
      trigger: "blur",
    },
  ],
});
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        await locationStore.registerComputer(
          props.electionGuid,
          props.locationGuid,
          dto,
        );
...
<template>
  <el-dialog
    ...
    :title="$t('locations.computer.title')"
    width="600px"
    ...
    >
      <el-form-item :label="$t('locations.computer.code')" prop="computerCode">
        <el-input
          ...
          :placeholder="$t('locations.computer.codePlaceholder')"
          maxlength="2"
          style="text-transform: uppercase"
        />
        <div class="form-help-text">
          {{ $t("locations.computer.codeHelp") }}
        </div>
      </el-form-item>

      <el-form-item
        :label="$t('locations.computer.browserInfo')"
        prop="browserInfo"
      >
        <el-input
          ...
          type="textarea"
          :rows="3"
          :placeholder="$t('locations.computer.browserInfoPlaceholder')"
          readonly
        />
        <div class="form-help-text">
          {{ $t("locations.computer.browserInfoHelp") }}
        </div>
      </el-form-item>

      <el-form-item
        :label="$t('locations.computer.ipAddress')"
        prop="ipAddress"
      >
        <el-input
          v-model="form.ipAddress"
          :placeholder="$t('locations.computer.ipAddressPlaceholder')"
        />
        <div class="form-help-text">
          {{ $t("locations.computer.ipAddressHelp") }}
        </div>
      </el-form-item>
    </el-form>
