...
class TokenRefreshService {
  ...
  startAutoRefresh(): void {
    ...
    const refreshIntervalMs = this.config.tokenExpiryMinutes * this.config.refreshThreshold * 60 * 1000;

    console.log(`[TokenRefresh] Starting auto-refresh every ${refreshIntervalMs / 60000} minutes`);

    // Refresh immediately if we haven't refreshed recently
    const timeSinceLastRefresh = this.lastRefreshTime ? Date.now() - this.lastRefreshTime : Infinity;
