...
export const resultService = {
  // Tally Calculation
  async calculateTally(electionGuid: string, electionType: 'normal' | 'singlename' = 'normal'): Promise<TallyResultDto> {
    const response = await api.post<TallyResultDto>(`/results/election/${electionGuid}/calculate`, null, {
      params: { electionType }
    });
  ...
  async getMonitorInfo(electionGuid: string): Promise<MonitorInfoDto> {
    const response = await api.get<MonitorInfoDto>(`/results/election/${electionGuid}/monitor`);
    return response.data;
  },

  // Reports
  async getElectionReport(electionGuid: string): Promise<ElectionReportDto> {
    const response = await api.get<ElectionReportDto>(`/results/election/${electionGuid}/report`);
    return response.data;
  },

  async getReportData(electionGuid: string, reportCode: string): Promise<ReportDataResponseDto> {
    const response = await api.get<ReportDataResponseDto>(`/results/election/${electionGuid}/report/${reportCode}`);
    return response.data;
  },

  // Tie Management
  async getTieDetails(electionGuid: string): Promise<TieDetailsDto[]> {
    const response = await api.get<TieDetailsDto[]>(`/results/election/${electionGuid}/ties`);
    return response.data;
  },

  async saveTieCounts(electionGuid: string, request: SaveTieCountsRequestDto): Promise<SaveTieCountsResponseDto> {
    const response = await api.post<SaveTieCountsResponseDto>(`/results/election/${electionGuid}/ties/save`, request);
    return response.data;
  },
