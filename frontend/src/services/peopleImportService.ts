...
export const peopleImportService = {
  async uploadFile(electionGuid: string, file: File): Promise<ImportFileInfo> {
    ...
    const response = await api.post<ImportFileInfo>(`/api/PeopleImport/${electionGuid}/upload`, formData, {
      headers: {
  ...
  async getFiles(electionGuid: string): Promise<ImportFileInfo[]> {
    const response = await api.get<ImportFileInfo[]>(`/api/PeopleImport/${electionGuid}/files`);
    return response.data;
  },

  async parseFile(electionGuid: string, rowId: number, codePage?: number, firstDataRow?: number): Promise<ParseFileResult> {
    const params = new URLSearchParams();
    if (codePage !== undefined) params.append('codePage', codePage.toString());
    if (firstDataRow !== undefined) params.append('firstDataRow', firstDataRow.toString());
  ...
  async saveMapping(electionGuid: string, rowId: number, mappings: ColumnMapping[]): Promise<ImportFileInfo> {
    const response = await api.put<ImportFileInfo>(`/api/PeopleImport/${electionGuid}/files/${rowId}/mapping`, mappings);
    return response.data;
  },
  ...
  async executeImport(electionGuid: string, rowId: number): Promise<ImportPeopleResult> {
    const response = await api.post<ImportPeopleResult>(`/api/PeopleImport/${electionGuid}/files/${rowId}/import`);
    return response.data;
  },
  ...
  async deleteAllPeople(electionGuid: string): Promise<DeleteAllPeopleResult> {
    const response = await api.delete<DeleteAllPeopleResult>(`/api/PeopleImport/${electionGuid}/people`);
    return response.data;
  },

  async getPeopleCount(electionGuid: string): Promise<{ count: number }> {
    const response = await api.get<{ count: number }>(`/api/PeopleImport/${electionGuid}/people-count`);
    return response.data;
  }
};