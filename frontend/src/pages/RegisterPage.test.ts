...
describe('RegisterPage', () => {
  ...
  describe('Password Validation', () => {
    it('validates password minimum length of 12 characters', async () => {
      ...
      await validatePassword(null, 'Short1!', (error: any) => {
        hasError = true
      })
    ...
    it('validates password requires lowercase letter', async () => {
      ...
      await validatePassword(null, 'PASSWORD123!', (error: any) => {
        hasError = true
      })
    ...
    it('validates password requires digit', async () => {
      ...
      await validatePassword(null, 'Password!', (error: any) => {
        hasError = true
      })
    ...
    it('accepts valid password that meets all requirements', async () => {
      ...
      await validatePassword(null, 'ValidPassword123!', (error: any) => {
        if (error) {
          hasError = true
    ...
    it('validates password confirmation matches', async () => {
      ...
      await validatePass2(null, 'DifferentPassword123!', (error: any) => {
        hasError = true
      })
  ...
  describe('Registration Flow', () => {
    it('calls auth store register method on form submission', async () => {
      ...
      const formRef = { validate: vi.fn().mockImplementation((callback) => callback(true)) }
