...
import type { BallotDto, ElectionDto, VoteDto, CreateVoteDto } from '@/types';
...
vi.mock('@/components/ballots/InlineBallotEntry.vue', () => ({
  default: {
    ...
    template: '<div class="inline-ballot-entry-mock" data-testid="inline-ballot-entry"></div>',
    props: {
      electionGuid: String,
      ballot: Object,
      maxVotes: Number
...
describe('BallotEntryPage', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(electionStore, 'fetchElectionById').mockImplementation(async () => {
  ...
  it('initializes stores on mount', async () => {
    ...
    expect(ballotStore.fetchBallotById).toHaveBeenCalledWith('test-ballot-guid');
    expect(electionStore.fetchElectionById).toHaveBeenCalledWith('test-election-guid');
  });
  ...
  it('leaves election groups on unmount', async () => {
    ...
    expect(ballotStore.leaveElection).toHaveBeenCalledWith('test-election-guid');
    expect(peopleStore.leaveElection).toHaveBeenCalledWith('test-election-guid');
  });
